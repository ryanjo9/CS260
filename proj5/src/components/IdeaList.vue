...
<div>
  ...
  <div class="idea" v-for="idea in selection" v-bind:key="idea._id">
      ...
      <p v-if='idea.directionsLink'><a v-bind:href="idea.directionsLink">Directions</a>---<a v-bind:href="idea.reviewsLink">Google Reviews</a></p>
      <p class="ideaDate">
        <span v-if="idea.user.name">{{idea.user.name}}, </span>
        {{formatDate(idea.created)}}
        <br>Likes: {{idea.numLikes}}
      </p>
      <button v-on:click="like(idea._id)"><i class="fas fa-thumbs-up"></i></button>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    async like(id) {
      await this.$store.dispatch("like", id)
    }
  }
}
</script>
